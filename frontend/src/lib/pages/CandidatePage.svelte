...
{:else}
  ...
  {#if memo}
    <section id="memo" class="memo-layout shell" aria-labelledby="memo-title">
      <div class="memo-main">
        <div class="section-title memo-heading"><div><p class="eyebrow">Investment memo · Generated {dateLabel(memo.generated_at)}</p><h2 id="memo-title">What the evidence supports</h2></div><Chip variant="status" tone="forest" label={`${Object.keys(memo.claims).length} claims cited`} icon="✓" /></div>
      ...
      <aside class="gaps-box" aria-labelledby="gaps-title"><div class="gaps-icon" aria-hidden="true">∅</div><p class="eyebrow">Evidence gaps</p><h2 id="gaps-title">Not observed</h2><p>These gaps remain intentionally blank. No proxy data or assumptions have been used.</p><ul>{#each memo.gaps as gap}<li><span>—</span>{gap}</li>{/each}</ul><footer>Required before investment committee</footer></aside>
    </section>
