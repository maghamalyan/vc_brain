...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--stage", required=True, choices=("extract", "annotate"))
    result.add_argument(
        "--mock",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.stage == "extract" and (args.mock or args.fixture_dir or args.actor_limit):
    ...
    if args.stage == "extract":
        ...
        frame = annotate_text_items(
            mock=args.mock,
            fixture_dir=args.fixture_dir,
            actor_limit=args.actor_limit,
        )
        print(f"Annotated {frame.height} person-quarters")
